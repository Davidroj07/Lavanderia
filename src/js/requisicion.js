...
async function obtenerDatos() {
    ...
    const res = await fetch(`api/requisiciones/${ano}`);
    data = await res.json();
    allTableData = data
    return data;
}
...
function renderTable() {
    ...
    tableBody.innerHTML = datoToShowToShow.map((dato, index) => `
        <tr>
            <td>${dato.numero}</td>
            <td>${dato.nombre}</td>
            <td>${funciones.formatearFecha(dato.fecha_recibido)}</td>
...
function editMember(index) {
    ...
    redireccionar.cargarContenido('/requisicionD', 'requisicionForm', () => {
    });
...
function filterTable(columnIndex) {
    data = funciones.filterTable("tablaRequisicion", columnIndex, allTableData);
    renderTable();
}
