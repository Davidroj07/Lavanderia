...
function renderPagination() {
    ...
    pagination.innerHTML = Array.from({ length: totalPages }, (_, i) => `
        <li><a href="#sucursal" class="${i + 1 === currentPage ? 'active' : ''}" onclick="changePage(${i + 1})">${i + 1}</a></li>
    `).join('');
...
async function deleteMember(index) {
    ...
    if (confirm(`¿Estás seguro de que deseas borrar la sucursal con ID ${datoId}?`)) {

            const apiUrl = `/api/sucursales/${dato.id}`;
...
function editMember(index) {
    ...
    document.getElementById('name').value = dato.nombre;
    document.getElementById('full-name').value = dato.nombre_completo;
    document.getElementById('nit').value = dato.nit;
    document.getElementById('address').value = dato.direccion;
    document.getElementById('phone').value = dato.telefono;
    document.getElementById('city').value = dato.ciudad;
...
document.getElementById('new-record-btn').onclick = () => {
    ...
    form.onsubmit = async (e) => {
        ...
        const id = await obtenerMaximo("sucursal");
        ...
        const sucursalData = {
            id: id,  // Asignamos el ID 4 por ahora
            nombre: nombre,
...
function filterTable(columnIndex) {
    data = funciones.filterTable("tablaSucursal", columnIndex, allTableData);
    renderTable();
}
